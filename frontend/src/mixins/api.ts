...
export class MMAPI {
  ...
  public static async getPublicNote(noteName: string): Promise<Note> {
    return (await MMAPI.GET(`${MMAPI.NOTE_URL}/${noteName}`)) as Note;
  }
   ...
   */
  public static async getPublicDesk(deskName: string): Promise<Desk> {
